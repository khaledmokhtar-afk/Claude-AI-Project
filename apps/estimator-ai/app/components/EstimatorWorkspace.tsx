...
export function EstimatorWorkspace({
  ...
  const runEstimate = async () => {
    ...
    if (mode === "demo") {
      ...
      setEstimate(buildDemoEstimate(selectedPreset || "wellhead-4"));
      setIsRunning(false);
      return;
    }
        ...
        <div className="flex flex-col gap-6 min-h-[70vh]">
            ...
            !isRunning && (
              <div className="glass p-10 text-center text-[var(--color-text-muted)]">
                ...
                <p className="text-sm max-w-md mx-auto mt-2">
                  Pick a preset or enter your own brief. Top-5 analog projects are retrieved live
                  from the historical corpus. Click <strong>Run estimate</strong> to produce bounds.
                </p>
              </div>
            )
          )}
          ...
          {error && (
            <div
              ...
              style={{ borderLeftColor: "var(--color-neg)" }}
            >
              <strong style={{ color: "var(--color-neg)" }}>Error:</strong> {error}
            </div>
          )}
        </div>
      </div>
...
async function streamNarration(
  ...
  const res = await fetch("/api/narrate-estimate", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ query, analogCount: analogs.length, analogTypes: analogs.map((a) => a.projectType) }),
  });
...
export const __HISTORICAL_COUNT = HISTORICAL_PROJECTS.length;
